...
from arize-experiment.cache.base import CacheEntry
from arize-experiment.cache.manager import CacheManager
from arize-experiment.config import Config, CacheConfig, VectorStoreType
from arize-experiment.providers.base import LLMProvider
...
def cache_manager(config, openai_client, provider, vector_store):
    ...
    patches = [
        patch("arize-experiment.cache.manager.PineconeVectorStore", return_value=vector_store),
        patch("arize-experiment.cache.manager.QdrantVectorStore", return_value=vector_store),
        patch("arize-experiment.cache.manager.PgVectorStore", return_value=vector_store),
    ]
...
def test_create_pinecone_store(config, openai_client):
    ...
    with patch("arize-experiment.cache.manager.PineconeVectorStore") as mock_pinecone:
        CacheManager(config, openai_client, MockProvider("fake_key"))
...
def test_create_qdrant_store(config, openai_client):
    ...
    with patch("arize-experiment.cache.manager.QdrantVectorStore") as mock_qdrant:
        CacheManager(config, openai_client, MockProvider("fake_key"))
...
def test_create_pgvector_store(config, openai_client):
    ...
    with patch("arize-experiment.cache.manager.PgVectorStore") as mock_pgvector:
        CacheManager(config, openai_client, MockProvider("fake_key"))
